	...
	protected function shellInitialize()
	{
		if ($this->getInitialized() === null) {
			...
			try {
				...
				$strCmd		= "set PROMPT=".$this->_shellPrompt."";
				$delimitor	= "(\n" . preg_quote($this->_shellPrompt) .")";
				$reData		= $this->exeCmd($strCmd, $delimitor);
				...
				if ($this->getParentShell() === null) {
					...
					//this is not a good way to do this, if 2 shells open at the same time
					//we might get the wrong one
					$strCmd		= "wmic process get parentprocessid,name | find \"WMIC\"";
					$reData		= $this->exeCmd($strCmd);
					
					$lines		= explode("\n", $reData);
					$lastPID	= null;
					foreach ($lines as $line) {
						if (preg_match("/WMIC.exe\s+([0-9]+)/i", $line, $rawPid) == 1) {
							$lastPID	= $rawPid[1];
						}
					}

					if ($lastPID !== null) {
						$this->_procPID	= $lastPID;
					} else {
						throw new \Exception(__METHOD__ . ">> Failed to get process id");
					}
				}
	...
	protected function shellTerminate()
	{
		if ($this->getInitialized() !== false) {
			...
			try {
				...
				//issue the exit
				$strCmd		= "exit";
				$this->exeCmd($strCmd, false, 0);
				
				//must delete its own files, the process is started by proc_open so we cannot stack commands
				//for some reason must be done AFTER the exit command or the files are not deleted
				$path		= $this->getPipes()->getOutputFile()->getDirectory()->getPathAsString();
				$deleteCmd	= "START \"deSeq\" cmd /c \"ping -n 2 127.0.0.1 & rmdir /s /q \"" .$path. "\"\"";
				exec($deleteCmd);
